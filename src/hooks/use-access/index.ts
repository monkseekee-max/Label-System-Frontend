import { hasAnyRole, isKnownRole } from "#src/router/role-hierarchy";
...
export function useAccess() {
	...
	const hasAccessByRoles = (roles?: string | Array<string>) => {
		...
		roles = roles.map(item => item.trim());
		if (import.meta.env.DEV) {
			const knownRoles = new Set<string>(Object.values(AccessControlRoles));
			// 校验角色是否合法，不合法的角色会打印警告信息
			for (const roleItem of roles) {
				if (!isKnownRole(roleItem) && !knownRoles.has(roleItem.toUpperCase()) && !knownRoles.has(roleItem)) {
					console.warn(`[hasAccessByRoles]: '${roleItem}' is not a valid role`);
				}
			}
		}
		const isAuth = hasAnyRole(userRoles, roles);
		return isAuth;
	};
