...
export const i18nInitOptions: InitOptions = {
	...
	missingKeyHandler: async (languages, namespace, translationKey) => {
		...
		// Skip warnings for keys that are already in the target language
		// (e.g. hardcoded Chinese menu titles passed through t() in zh-CN mode)
		const isCJK = /[\u4e00-\u9fff\u3040-\u30ff\uac00-\ud7af]/.test(translationKey);
		if (currentLanguage === "zh-CN" && isCJK) {
			return;
		}
