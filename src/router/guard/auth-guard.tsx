...
import { changePasswordPath, exception403Path, exception404Path, exception500Path, loginPath } from "#src/router/extra-info";
...
export function AuthGuard({ children }: AuthGuardProps) {
	...
	if (!isLogin) {
		...
		// 未登录且目标页不是登录页/官网, 则跳转到登录页
		if (pathname !== loginPath && pathname !== "/landing") {
	...
	/**
	 * @zh 强制修改密码：用户首次登录或管理员重置密码后，必须先修改密码
	 * @en Force password change: user must change password on first login or after admin reset
	 */
	 ...
	 */
	const hasRoutePermission = hasRouteAccessByRoles(userRoles, routeRoles);
	/**
	 * @zh 权限校验逻辑：
