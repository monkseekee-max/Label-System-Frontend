...
export async function generateRoutesFromBackend(backendRoutes: Array<AppRouteRecordRaw>) {
	...
	const loadRouteComponent = async (route: AppRouteRecordRaw, componentPath: string) => {
		...
		if (moduleIndex !== -1) {
			// import.meta.glob 返回 () => Promise<unknown>; 此处只 glob .tsx 页面组件,
			// 收窄为 React.lazy 期望的 () => Promise<{ default: ComponentType }> 加载器
			const loader = pageModules[pageModulePaths[moduleIndex]] as () => Promise<{ default: ComponentType }>;
			route.Component = lazy(loader);
		}
		else {
